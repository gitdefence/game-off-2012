function Bug(startPath) {
    ...
    this.update = function (dt) {
        ...
        if (overshot) this.tPos.update(-dt);
    ...
    this.draw = function (pen) {
        ...
        DRAW.arc(pen, cen, this.radius + this.lineWidth, 0, Math.PI * 2 * hpPercent, new HSLColor().h(hue).s(50).l(50).a(0).str(), 3, new HSLColor().h(hue).s(90).l(60).a(1).str());

        DRAW.circle(pen, cen, this.radius, this.color, 1, this.borderColor);

        DRAW.circle(pen, cen, this.attr.range, setAlpha(this.color, 0.13));
    }
}