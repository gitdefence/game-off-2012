...
function vecToRect(point, rect, vector) {
    if (!assertDefined("vecToRect", rect, point) || !assertRectangle(rect)) return new Vector(0, 0);

    if (!vector) vector = new Vector(0, 0);

    if (point.x > (rect.x + rect.w)) vector.x = (rect.x + rect.w) - point.x;
    else if (point.x < rect.x) vector.x = rect.x - point.x;
    else vector.x = 0;

    if (point.y > (rect.y + rect.h)) vector.y = (rect.y + rect.h) - point.y;
    else if (point.y < rect.y) vector.y = rect.y - point.y;
    else vector.y = 0;
...
function minVecToOuterRect(point, rect, vector) {
    ...
    if (point.x < cenX) vector.x = rect.x - point.x;
    else vector.x = (rect.x + rect.w) - point.x;

    var cenY = rect.y + rect.h / 2;
    if (point.y < cenY) vector.y = rect.y - point.y;
    else vector.y = (rect.y + rect.h) - point.y;
...
function minVecBetweenRectsOneWay(rectOne, rectTwo) {
    if (!assertDefined("minVecBetweenRects", rectOne, rectTwo) || !assertRectangle(rectOne) || !assertRectangle(rectTwo)) return new Vector(0, 0);
    ...
    if (distance2.magSq() < minimum.magSq()) minimum = distance2;

    if (distance3.magSq() < minimum.magSq()) minimum = distance3;

    if (distance4.magSq() < minimum.magSq()) minimum = distance4;
...
function minVecForDistanceRects(rectOne, rectTwo, distance) {
    if (!assertDefined("minVecForDistanceRects", rectOne, rectTwo, distance) || !assertRectangle(rectOne) || !assertRectangle(rectTwo)) return new Vector(0, 0);
...
function minVecFullOverlapRects(rectOne, rectTwo) {
    if (!assertDefined("minVecFullOverlapRects", rectOne, rectTwo) || !assertRectangle(rectOne) || !assertRectangle(rectTwo)) return new Vector(0, 0);
    ...
    if (distance2.magSq() > maximum.magSq()) maximum = distance2;

    if (distance3.magSq() > maximum.magSq()) maximum = distance3;

    if (distance4.magSq() > maximum.magSq()) maximum = distance4;
...
function vecBetweenRectAndCircle(circle, circle) {
    if (!assertDefined("vecBetweenRectAndCircle", rect, circle) || !assertRectangle(rect) || !assertRectangle(circle)) return new Vector(0, 0);

    var vec = vecToRect(getCircleCenter(circle), rect);

    if (vec.magSq() < circle.w * circle.w) return new Vector(0, 0);
    else return dist;
}
