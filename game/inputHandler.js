...
function InputHandler() {
    ...
    function throwMouseEventAt(mX, mY, eventName, eng, ignore) {
        ...
        for (var type in eng.base.allChildren) {
            mergeToArray(findAllWithin(eng, type, {
                x: mX,
                y: mY
            }, 0), allUnderMouse);
        }

        if (allUnderMouse.length == 0) return;
        ...
        if (!ignore[key]) if (allUnderMouse[key].base.zindex > topMost.base.zindex || (allUnderMouse[key].base.zindex == topMost.base.zindex && allUnderMouse[key].base.zoffset > topMost.base.zoffset)) {
            if (allUnderMouse[key].base.canHandleEvent(eventName)) topMost = allUnderMouse[key];
        }

        for (var key in allUnderMouse) {
            if (!ignore[key]) {
                if (allUnderMouse[key] !== topMost) {
                    allUnderMouse[key].base.callRaise(eventName, {
                        x: mX,
                        y: mY,
                        topMost: false
                    });
        ...
        topMost.base.callRaise(eventName, {
            x: mX,
            y: mY,
            topMost: true
        });
    ...
    this.handleEvents = function (eng) {
        ...
        if (this.resizeEvent) {
            ...
            if (this.consumeEvents) this.resizeEvent = null;
        }
    };
    ...
    this.handleMouseEvents = function (eng) {
        if (this.mdX > 0 && this.mdY > 0) {
            for (var key in this.globalMouseDown) {
                if (this.globalMouseDown[key].base.rootNode != eng) delete this.globalMouseDown[key];
                else this.globalMouseDown[key].base.callRaise("mousedown", {
                    x: this.mdX,
                    y: this.mdY
                });
            }
            ...
            for (var key in this.globalMouseUp) {
                if (this.globalMouseUp[key].base.rootNode != eng) delete this.globalMouseUp[key];
                else this.globalMouseUp[key].base.callRaise("mouseup", {
                    x: this.muX,
                    y: this.muY
                });
            }
            ...
            if (this.prevMouseDown && this.prevMouseDown.length > 0) {
                for (var key in this.globalMouseClick) {
                    if (this.globalMouseClick[key].base.rootNode != eng) delete this.globalMouseClick[key];
                    else this.globalMouseClick[key].base.callRaise("click", {
                        x: this.muX,
                        y: this.muY
                    });
                }
                ...
                for (var i = 0; i < this.prevMouseDown.length; i++) {
                    if (!this.globalMouseClick[this.prevMouseDown[i].base.id] && vecToRect({
                        x: this.muX,
                        y: this.muY
                    }, this.prevMouseDown[i].tPos).magSq() == 0) {
                        this.prevMouseDown[i].base.callRaise("click", {
                            x: this.muX,
                            y: this.muY
                        });
                    }
                    this.prevMouseDown[i].base.callRaise("dragEnd", {
                        x: this.muX,
                        y: this.muY
                    });
                }
        ...
        if (this.mY > 0 && this.mX > 0) {
            for (var key in this.globalMouseMove) {
                if (this.globalMouseMove[key].base.rootNode != eng) delete this.globalMouseMove[key];
                else this.globalMouseMove[key].base.callRaise("mousemove", {
                    x: this.mX,
                    y: this.mY
                });
            }
            ...
            if (this.prevMouseOver && this.prevMouseOver.length > 0) {
                for (var i = 0; i < this.prevMouseOver.length; i++) {
                    if (vecToRect({
                        x: this.mX,
                        y: this.mY
                    }, this.prevMouseOver[i].tPos).magSq() != 0) {
                        this.prevMouseOver[i].base.callRaise("mouseout", {
                            x: this.mX,
                            y: this.mY
                        });
                    }
            ...
            if (this.prevMouseDown && this.prevMouseDown.length > 0) {
                for (var i = 0; i < this.prevMouseDown.length; i++) {
                    this.prevMouseDown[i].base.callRaise("dragged", {
                        x: this.mX,
                        y: this.mY
                    });
                }
